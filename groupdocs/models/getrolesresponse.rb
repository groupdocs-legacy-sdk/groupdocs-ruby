class GetRolesResponse
  ...
  def initialize(attributes = {})
    ...
    if attributes.to_s != ""
      ...
      if GetRolesResponse.attribute_map["result".to_sym] != nil
        name = "result".to_sym
        value = attributes["result"]
        send("#{name}=", value) if self.respond_to?(name)
	      end
      if GetRolesResponse.attribute_map["status".to_sym] != nil
        name = "status".to_sym
        value = attributes["status"]
        send("#{name}=", value) if self.respond_to?(name)
	      end
      if GetRolesResponse.attribute_map["error_message".to_sym] != nil
        name = "error_message".to_sym
        value = attributes["error_message"]
        send("#{name}=", value) if self.respond_to?(name)
	      end
      if GetRolesResponse.attribute_map["composed_on".to_sym] != nil
  ...
  def to_body
    body = {}
    GetRolesResponse.attribute_map.each_pair do |key,value|
